...
describe('Scenario Fixture Tests', () => {
  ...
  scenarios.forEach((scenario) => {
    it(`should execute ${scenario} scenario correctly`, () => {
      ...
      // Read files
      const inputFiles = readdirSync(scenarioDir).filter((f) =>
        f.startsWith('input.'),
      );
      ...
      const expected = readFileSync(
        join(scenarioDir, `expected.${extension}`),
        'utf-8',
      );
      ...
      const result =
        format === 'plantuml' ? builder.toPlantUML() : builder.toMermaid();

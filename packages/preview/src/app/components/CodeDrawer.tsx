...
export default function CodeDrawer({ sourceCode, transformedCode }: CodeDrawerProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<'source' | 'transformed'>('source');

  return (
    <div className="code-drawer">
      <button className="code-drawer-toggle" onClick={() => setIsOpen(!isOpen)}>
        <span>{isOpen ? '▼' : '▶'}</span>
        <span>コードを表示</span>
      </button>

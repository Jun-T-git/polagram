...
export default function LayerToggle({ layers, enabledLayers, onToggle }: LayerToggleProps) {
  const getLayerLabel = (layer: LayerItem): string => {
    ...
    if (selector.name) {
      target = typeof selector.name === 'string' ? selector.name : selector.name.pattern;
    } else if (selector.condition) {
      target = typeof selector.condition === 'string' ? selector.condition : selector.condition.pattern;
